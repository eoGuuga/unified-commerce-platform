...
Ultima revisao: 2026-05-15. Reler trimestralmente ou apos mudanca
de stack.
...
|---|---|---|
| **RPO** (Recovery Point Objective) | **24h** | Backup diario `backup-postgres.sh` rodando via cron as 03:00 UTC. Pior cenario: perda de dados do dia em curso. |
...
```
+--------------+   pg_dump+gzip   +---------------+   rclone+criptografia   +-------+
|  Postgres    | ---------------> | /opt/ucm/     | ----------------------> | B2    |
|  (prod)      | (cron 03:00 UTC) | backups/      | (cron 03:30 UTC)        | crypt |
+--------------+                  +---------------+                         +-------+
                                      |                                          |
                                      | (retencao 7d local)                      | (retencao 30d offsite)
                                      v                                          v
                                  rm -mtime +7                              rclone delete --min-age 30d
```
...
- `deploy/scripts/backup-offsite.sh`: rclone sync do `${BACKUP_DIR}`
  para B2 (Backblaze) usando remote criptografado (`b2crypt:`).
...
4. Restaurar para banco temporario para inspecao (NUNCA sobrescrever
   ...
   gunzip -c /opt/ucm/backups/ucm-<TIMESTAMP>.sql.gz | \
...
- Conta B2 + rclone config (`~/.config/rclone/rclone.conf` com
  remote `b2crypt:` configurado).
...
5. Subir stack:
   ...
   ```
6. Pegar o backup mais recente do B2:
   ```bash
   rclone copy b2crypt: /opt/ucm/backups/ \
     --include "ucm-*.sql.gz" --max-age 25h --min-age 0
   ```
7. Restaurar (CUIDADO, vai sobrescrever - DB esta vazia neste cenario):
   ```bash
   latest=$(ls -1t /opt/ucm/backups/ucm-*.sql.gz | head -1)
   gunzip -c "$latest" | \
...
|---|---|---|
| Diaria | Cron `backup-postgres.sh` 03:00 UTC; cron `backup-offsite.sh` 03:30 UTC | Automatico |
| Diaria | Verificar log do offsite: `tail -50 /var/log/ucm-backup-offsite.log` | Solo dev (manual ou alerta) |
| Mensal | `restore-drill-offsite.sh` valida que B2 esta restoravel + envia alerta Telegram em falha | Cron 1o do mes 04:00 UTC |
