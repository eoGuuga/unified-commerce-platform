...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'Cadastro público - cria tenant e usuário',
    description: 'Endpoint público para novos usuários criarem conta e tenant automaticamente.',
  })
  @ApiResponse({ status: 201, description: 'Conta criada com sucesso' })
  @ApiResponse({ status: 400, description: 'Dados inválidos' })
  @ApiResponse({ status: 409, description: 'Email já cadastrado' })
  async signup(@Body() registerDto: RegisterDto): Promise<LoginResponse> {
  ...
  async sendConfirmation(@Body() body: { email: string }) {
    return this.authService.sendConfirmationEmail(body.email);
  }

  @Post('confirm-email')
  @Public()
  async confirmEmail(@Body() body: { email: string; code: string }) {
