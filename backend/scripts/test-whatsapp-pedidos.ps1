...
$testes = @(
    ...
    # Erros Comuns (deve encontrar mesmo com erro)
...
foreach ($teste in $testes) {
    ...
    try {
        ...
        $status = if ($resultadoReal -eq $esperado) { "✅" } else { "❌" }
        ...
        Write-Host "$status [$resultadoReal] '$mensagem'" -ForegroundColor $(if ($status -eq "✅") { "Green" } else { "Red" })
...
Write-Host "Resultados exportados para arquivo JSON" -ForegroundColor Cyan
