...
export class WhatsappService {
  ...
  // Prompts e formatadores: implementacoes em utils/order-formatters.ts.
  ...
  // utils/order-formatters.ts. Aliases mantem o codigo legado funcionando.
  private buildTrackingUrl(orderNo: string): string {
    return buildTrackingUrlUtil(orderNo, this.config.get<string>('FRONTEND_URL'));
  }

  private getOrdersPortalUrl(): string {
    return getOrdersPortalUrlUtil(this.config.get<string>('FRONTEND_URL'));
  }

  private getGreetingLine(customerName?: string | null): string {
    return getGreetingLine(customerName);
  }
  ...
  private getNextStepSummary(pedido: Pedido, status: PedidoStatus): string {
    return getNextStepSummary(pedido, status);
  }

  private getOrderStatusAction(pedido: Pedido, status: PedidoStatus): string | null {
    return getOrderStatusAction(pedido, status);
  }
