...
Criar `frontend/components/admin/shell/AdminDataProvider.tsx` (`'use client'`): um React context que chama **uma** `useOrders()` e expõe `{ orders, ordersLoading, ordersError, refetchOrders, updateOrderStatus, updatingOrderId, pedidosCount }`. `pedidosCount` = nº de pedidos em status novo/pendente (filtrar `orders` pelos status que contam pro selo — ex.: `pendente_pagamento`/`confirmado`; alinhar com o que o OrdersManager considera "novo"). Hook de consumo: `export function useAdminData()` (lança se fora do provider).
- **Tipo do contexto completo desde a T3 (build standalone):** o `AdminDataContextValue` declara **também** os campos de estoque (`summary`, `stockLoading`, `stockError`, `attentionCount`, `history`, `adjustStock`, `setMin`) — na T3 com valores stub explícitos (`attentionCount: undefined`, `summary: null`, `history: async () => ({items:[],total:0})`, etc.), **não ausentes**. Assim `useAdminData().attentionCount` tipa e a T3 builda sozinha; a T5a só preenche os stubs com o `useStock` real. (Sem isso, o `npm run build` da T3 quebra com erro de tipo que parece não-relacionado.)
...
Em `AdminDataProvider.tsx` (criado na T3 com orders), **adicionar** uma chamada a `useStock()` e preencher os campos de estoque do contexto (antes stub): `summary`, `stockLoading`, `stockError`, `refetchStock`, `attentionCount`, `history`, e (T5b) `adjustStock`/`setMin`. Agora o provider segura **uma** instância de cada hook; todos os consumidores (selo, Início, StockManager) leem o **mesmo** estado — o optimistic de um ajuste (T5b) re-renderiza os três e reverte junto. (Este é o coração da correção: fonte única no dado, não só na função.)
- **`stockError` NÃO-FATAL (guarda do provider):** o provider centralizou os fetches, então um erro do `getStockSummary` (ex.: tenant sem estoque que 500e) **não pode derrubar a casca** — Pedidos e Produtos não têm nada com estoque. Tratamento: erro de stock → `attentionCount` vira `undefined` (selo de Estoque some) e a tela de Estoque mostra seu próprio estado de erro com retry; o `AdminShell`/`AdminNav` e as demais telas seguem vivos. Confirmar na implementação que `getStockSummary` num tenant **sem produtos/estoque** devolve **lista vazia** (não 404/500); se devolver erro, blindar o provider pra degradar o selo, não a casca.
