...
- Producao (VPS): `/opt/ucm/deploy/.env` (use `deploy/env.prod.example` como base).
- Dev/test (VPS): `/opt/ucm-test-repo/deploy/.env` (use `deploy/env.dev.example`).
...
Detalhe completo em: `docs/LEGADO/07-setup/PEGAR-CREDENCIAIS-OFICIAIS.md`.
