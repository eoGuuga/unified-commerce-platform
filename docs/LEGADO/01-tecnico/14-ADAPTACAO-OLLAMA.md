...
# ðŸ¤– AdaptaÃ§Ã£o para Ollama (IA Gratuita)

> **Guia prÃ¡tico:** Como adaptar o cÃ³digo para usar Ollama em vez de OpenAI.
...
```bash
# OpÃ§Ã£o 1: Modelo pequeno e rÃ¡pido
ollama pull llama3.2:3b

# OpÃ§Ã£o 2: Melhor qualidade
ollama pull mistral:7b

# OpÃ§Ã£o 3: Equilibrado
...
# Terminal 2: Testar
ollama run llama3.2:3b "OlÃ¡, tudo bem?"
```
...
#### OpÃ§Ã£o A: Modificar existente (OpenAI Service)
  ...
  private initializeOpenAI() {
    ...
    if (useOllama) {
      ...
      this.openai = new OpenAI({
        baseURL: ollamaUrl,
        apiKey: 'ollama', // NÃ£o precisa, mas API requer
      });
    ...
    try {
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'VocÃª Ã© um assistente de vendas de doces artesanais. Responda em portuguÃªs brasileiro.',
          },
          {
            role: 'user',
            content: message,
...
#### OpÃ§Ã£o B: Criar Ollama Service separado
  ...
  async processMessage(message: string): Promise<MessageIntent> {
    try {
      const response = await fetch(`${this.baseURL}/api/generate`, {
        ...
        body: JSON.stringify({
          model: this.model,
          prompt: `VocÃª Ã© um assistente de vendas de doces artesanais. 
          ...
          - "consultar" se quer informaÃ§Ãµes
          - "unknown" se nÃ£o entender
...
## ðŸ“Š ComparaÃ§Ã£o: Ollama vs OpenAI

| Aspecto | Ollama | OpenAI |
|---------|--------|--------|
| **Custo** | âœ… **GrÃ¡tis** | âŒ ~$0.01-0.03/msg |
| **LatÃªncia** | âš ï¸ 1-3s (local) | âœ… 0.5-1s (cloud) |
| **Qualidade** | âš ï¸ Boa (7b) | âœ… Excelente (GPT-4) |
| **ConfiguraÃ§Ã£o** | âš ï¸ Precisa instalar | âœ… SÃ³ API key |
| **PortuguÃªs** | âš ï¸ Funciona | âœ… Excelente |
| **RecomendaÃ§Ã£o** | âœ… **Desenvolvimento** | âœ… **ProduÃ§Ã£o** |
...
- **llama3.2:3b** - RÃ¡pido, leve, desenvolvimento
- **mistral:7b** - Melhor qualidade, ainda rÃ¡pido
...
- PeÃ§a resposta estruturada (JSON se possÃ­vel)
