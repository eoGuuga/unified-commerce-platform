...
3. **⬜ 🔴 🔧 Observabilidade — PRÓXIMO 🔴 (diagnóstico parcial já na mão — retomar em sessão fresca).**
   **O que a investigação do backup (2026-07-06) já descobriu** — a mudança de path `/opt/ucm → /opt/gtsofthub` da faxina deixou a automação de monitoramento MORTA, apontando pro path velho e falhando em loop:
   - **`ucm-watchdog.service`** (systemd, healthcheck + auto-recovery) → `cd: /opt/ucm: No such file or directory`, **exit 1 a cada ~1min** (visto no syslog). O que deveria auto-recuperar prod está morto.
   - **3 crons do usuário `ubuntu`**, todos no `/opt/ucm` inexistente: `monitor.sh` (a cada 30min, logava em `/var/log/ucm/monitor.log`), `disk-alert.sh` (de hora), `security-alert.sh` (a cada 15min).
   - **Receita = a mesma do backup:** repontar pro `/opt/gtsofthub` + **versionar no git** (o watchdog em `deploy/`, os crons em `/etc/cron.d` versionado como fizemos com `gtsofthub-backup`) → config de servidor deixa de "viver só no host" e sobrevive a redeploy.
   **O gap que o watchdog/crons NÃO cobrem (o alvo real de observabilidade):** o **netdata** existe (infra: CPU/disco/mem — visto no Bloco G), mas **não há alerta de APLICAÇÃO** — ninguém é avisado quando **um pagamento falha, o bot trava, ou sobe um pico de 5xx**. Erros de app só vão pro `docker logs`. Montar o mínimo: agregação de log + alerta em 5xx/pagamento-falho/bot-travado, sem esperar o lojista reclamar.
   **Como retomar:** diagnóstico só-leitura primeiro (ler o `ucm-watchdog.sh` + os 3 scripts pra saber o que faziam; ver o que o netdata já alerta; achar o gap real) → plano → conserto com aprovação. Mesma régua de servidor (só-leitura na investigação, mudança via git, reversível).
