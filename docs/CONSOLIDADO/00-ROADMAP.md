...
- 🟡 **Dívida F (deploy.sh auto-blindar o watchdog) — TESTADA, NÃO aplicada.** Fix desenhado: `trap ensure_watchdog_running EXIT` + `trap 'exit N' INT/TERM/HUP` religa o watchdog em QUALQUER saída (preserva o exit code real via `rc=$?`, `set +e` no trap, confirma `is-active` e grita se não voltou). **Testes verdes:** A 25/25 (stub: success/fail/INT/TERM/HUP) + B (prova no `ucm-watchdog.timer` REAL: deploy falho → trap religou → `active`). **Plano precisa de revisão pós-incidente:** o recovery/deploy que recria o nginx tem que tratar o conflito de :80 (a landmine acima). Aplicar só após o item (1).
