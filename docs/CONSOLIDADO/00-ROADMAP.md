...
- ✅ **Dívida F (deploy.sh auto-blindar o watchdog) — APLICADA (2026-07-09), em terreno limpo (landmine masked).** `trap ensure_watchdog_running EXIT` + `trap 'exit N' INT/TERM/HUP`: religa o watchdog em QUALQUER saída (preserva o exit code via `rc=$?`, `set +e` no trap, confirma `is-active` e grita `ALERTA CRITICO` se não voltou); passo `0/4` para o watchdog (best-effort, após os pre-checks). Tira o stop/start manual de todo deploy. **Testes:** A 25/25 (stub: success/fail/INT/TERM/HUP) + B (prova no `ucm-watchdog.timer` REAL). Validada ao vivo por um deploy no-op (watchdog religou sozinho pelo trap).
- 🟡 **Recovery robusto (item NOVO, do incidente) — `up -d` simples pode deixar container de rede QUEBRADA.** Tanto o auto-recovery do watchdog (`ucm-watchdog.sh:75` `up -d` + `:76` `docker restart`) quanto o `deploy.sh:42` (`up -d`) usam `up -d` simples — que, num container que falhou o start, deixa `NetworkSettings.Ports` vazio + DNS `127.0.0.11` recusado (provado no incidente; só `--force-recreate` limpou). **Fix = `--force-recreate` TARGETED / health-gated, NÃO cego** — um force-recreate cego recria TODOS os containers em todo deploy/recovery, incluindo `postgres`/`redis` → reinicia o banco = pior que o problema. **Baixa urgência:** a landmine masked removeu a causa-raiz do estado quebrado (a corrida de :80). Precisa design próprio.
