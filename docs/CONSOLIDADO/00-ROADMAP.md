...
1. **🔄 🔴 🔧 Backup — em conserto** ([runbook](../superpowers/plans/2026-07-06-backup-fix.md)). Diagnóstico 2026-07-06: **prod sem backup automático** — o `backup-postgres.sh` nunca foi agendado (a causa NÃO é o script; é cron ausente). Único backup = dumps manuais de Jul 3-4 (~2,5d atrás), válidos mas nunca restaurados. Conserto aprovado: backup fresco → provar restore no `ucm-postgres-test` → fix do script (path derivado, mata o F20) → cron versionado em `/etc/cron.d` (03:00 BRT, rotação 14d). **Fast-follow:** cópia **off-site** dos dumps (hoje no mesmo disco da VPS = não protege contra a VPS morrer). *(Achado colateral: watchdog + 3 crons de monitoramento mortos apontando pro `/opt/ucm` velho → entra na frente de Observabilidade abaixo.)*
2. **⬜ 🔴 👤+🔧 Custódia da `ENCRYPTION_MASTER_KEY`.** Se ela se perder, **toda credencial de pagamento + token de WhatsApp cifrado vira irrecuperável**. Ação (do dono, guiada por nós): **cópia segura FORA do servidor** (cofre de senha) + plano de rotação. (KMS é Fase 4; a custódia é HOJE.)
