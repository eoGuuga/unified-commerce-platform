...
> Dois achados de 2026-07-07 (a régua "verifica, não confia" pescou os dois contra suposições nossas). São pré-requisitos, não notas de rodapé:
>
> 1. **🔴 LANDMINE — auto-deploy stale.** `.github/workflows/deploy-prod.yml` dispara **deploy automático** em push que toca `backend/**` ou `frontend/**`, e sobe container com **env incompleto → quebra o boot em prod**. Docs **não** disparam ele (por isso o merge da doc foi seguro). **Antes do PRÓXIMO merge de backend/frontend (cupom pós-teste, ou pagamentos na Fase 4), a PRIMEIRA coisa é DESARMAR ou corrigir esse workflow.** É o tipo de armadilha que causa desastre silencioso: você mergeia algo inocente e o sistema faz um deploy quebrado sozinho.
...
4. **⬜ 🔴 🔧 LANDMINE — desarmar o auto-deploy `.github/workflows/deploy-prod.yml` (2026-07-07).** Workflow **stale** que faz `docker run` de imagem ghcr com env INCOMPLETO (falta `ENCRYPTION_KEY` etc.) num push que toca `backend/**`/`frontend/**` → **quebraria o boot em prod, sozinho, sem o dono mandar**. O deploy REAL é manual/compose (`deploy-backend`), então esse workflow é lixo perigoso. **BLOQUEADOR: é a PRIMEIRA coisa a tratar (desabilitar/remover/corrigir) antes de QUALQUER merge de código** (cupom pós-teste, pagamentos na Fase 4, bot pós-Meta). Merges de doc não o disparam (por isso o de hoje foi seguro). *(Só-leitura até aqui; conserto pendente de decisão do dono: apagar o arquivo vs. gatear com `if: false`/branch protegida.)*
