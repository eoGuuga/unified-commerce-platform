...
- 🟡 **Camada 5 — `http-exception.filter` vaza a query string em 4xx** (achado 2026-07-07 na ativação do bot). O filtro põe `request.url` (com os params da query) na **resposta 4xx E no log warn** (a máscara do filtro só cobre ≥500). Param sensível na URL vaza → o `hub.verify_token` do webhook Meta vazou por isso num 403 de teste (**risco baixo aceito pelo dono**: o verify token só serve pro handshake; a segurança real é o App Secret/HMAC — token NÃO rotacionado). **Fix futuro:** redigir params sensíveis (`hub.verify_token`, `token`, `access_token`…) na URL logada/devolvida pelo filtro. Mesmo tema das Camadas 1-4 de PII em log ([[security-pii-logs-fix]]).
