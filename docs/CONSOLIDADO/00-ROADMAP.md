...
1. **⬜ 🔴 🔧 Backup — confirmar que gera de verdade + 1 restore real.** O `backup-postgres.sh` existe, mas o F20 sugere que o path default quebra o `mkdir` → **pode não estar gerando dump nenhum**, e nunca foi restaurado. Ação: verificar se há dump recente + fazer **um restore de teste** de ponta a ponta.
