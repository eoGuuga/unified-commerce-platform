...
3. **🔄 🔴 🔧 Observabilidade — EM ANDAMENTO** ([runbook](../superpowers/plans/2026-07-06-observability.md)). Raiz: a faxina de path matou a automação de monitoramento + prod estava **cego na entrega** (nada chegava ao dono). **Feito 2026-07-06:**
   ...
   **PENDENTE (próxima sessão) — o 🔴 NÃO está 100% fechado:**
   - ⬜ **netdata → Telegram**: o netdata roda mas é **dashboard passivo** (config stock, sem MTA, bind `127.0.0.1`, 0 canal) → ligar o método Telegram nativo pra entregar disco cheio/memória/container-down.
   - ⬜ **3 crons mortos** (`monitor.sh`/`disk-alert.sh`/`security-alert.sh`) — os **scripts se perderam** (nunca versionados, irrecuperáveis): **remover** os crons mortos do crontab do `ubuntu` (funções cobertas por watchdog+netdata); `security-alert` = revisita se o dono lembrar o que fazia.
   - ⬜ **Alerta de app (Tier 1-lite)**: cron `docker logs --since 5m` → grep pagamento/webhook/5xx → Telegram (o "pagamento falhou, ninguém sabe"). **Tier 2** (notificador semântico in-app + APM Loki/Sentry) fica registrado pra depois.
