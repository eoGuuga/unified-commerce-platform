...
- 🟢 **Camada 5b — mensagem auto-404 do NestJS ecoa a query** (achado 2026-07-08 na validação da 5). Para rota **inexistente**, o NestJS gera `"Cannot GET /url?query"` e a query vaza pelo **`message`** (resposta + log), não pelo `path` (esse já limpo). **Vetor menor:** rota sem handler, a query é do próprio caller, sem segredo de servidor. **Fix futuro** (novo ciclo TDD+revisão, não hot-fix): no filtro, redigir a query do raw `request.url` quando ela aparecer no `message`, ou sobrescrever o handler 404.
